...
Saved addresses, schedules, preferences, local report history and collection outcomes are stored on the device. Postcode, council provider ID and selected opaque property reference are sent only for a requested live lookup. If the explicitly experimental Bin Day nationwide fallback is used, it also receives the selected street address because its collection endpoint requires it and may retain or cache the lookup under its own privacy policy; the What Bin gateway does not persist that address in its application database. Optional accounts keep email identity and plan/grant records in Supabase. Opt-in household sharing stores a nickname, public council identifier, member display names and explicit bin actions without uploading the address or postcode. Account export and eligible What Bin account-data removal are available from Account. Removal signs out only the current device, retains a minimal suppression marker so delayed billing events do not recreate access, detaches retained provider ledgers and retains the shared Supabase identity so access used by another product is not deleted. Starting an explicit Plus purchase/restore records a short-lived hashed intent; only a matching successful provider event clears suppression, while cancellation or abandonment does not. Underlying identity deletion is an assisted Help and support request. There is no third-party advertising or behavioural-tracking SDK. Relevant sponsored service listings, when a council has approved and enabled them, are labelled and can be hidden; pseudonymous listing/open events are recorded by the first-party gateway.

If a council has enabled and approved a bulky-waste partner, the app always shows the official council route and reuse guidance before the clearly labelled sponsored paid option. The first-party ledger stores a random installation ID, public council ID, partner, item, quantity, amount, platform fee, pseudonymous booking reference and status. Stripe Checkout collects the fulfilment contact and collection address; What Bin does not copy those details into its ledger. The platform records payment, provider acceptance, scheduling, completion, operator payout and any refund as separate events and does not report a click or checkout start as revenue.

The build supports iPad and adaptive portrait/landscape layouts. Physical iPad, rotation, Split View, large-text and VoiceOver acceptance are still outstanding and must be completed before this note is submitted.
