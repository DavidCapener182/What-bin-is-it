        ...
        <section className="data-list" aria-label="Council announcements">
          {items.length ? items.map((item) => (
            <article className="data-card" key={item.id}>
              ...
              {canPublish && item.status !== "archived" ? <div className="data-card-actions"><form action={changeAnnouncementStatusAction} className="stack-form"><input name="expectedOrganisationId" type="hidden" value={session.organisation.id} /><input name="id" type="hidden" value={item.id} />{item.status !== "published" ? <label className="check-option"><input name="sendPush" type="checkbox" value="yes" />Push alert</label> : null}{item.status !== "published" ? <label className="check-option"><input name="confirmAudience" type="checkbox" value="yes" />Audience checked</label> : null}{item.status !== "published" ? <label className="council-action-confirmation compact-confirmation"><input name="confirmCouncilAction" type="checkbox" value="yes" /><span><strong>Publish for {session.organisation.name}</strong><small>Confirm this is the intended council workspace.</small></span></label> : null}<div className="inline-form">{item.status !== "published" ? <button className="primary-button button-small" name="status" type="submit" value="published">Publish</button> : null}<button className="secondary-button button-small" name="status" type="submit" value="archived">Archive</button></div></form></div> : null}
            </article>
