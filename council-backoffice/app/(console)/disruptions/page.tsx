      ...
      <div className={canWrite ? "split-layout" : ""}>
        ...
        <section className="data-list">
          {items.length ? items.map((item) => <article className="data-card" key={item.id}>
            ...
            {canPublish && item.status !== "archived" ? <div className="data-card-actions"><form action={changeDisruptionStatusAction} className="inline-form"><input name="id" type="hidden" value={item.id} />{item.status === "draft" ? <label className="check-option"><input name="sendPush" type="checkbox" value="yes" />Push alert</label> : null}{item.status === "draft" ? <label className="check-option"><input name="confirmAudience" type="checkbox" value="yes" />Audience checked</label> : null}{item.status === "draft" ? <button className="primary-button button-small" name="status" value="published">Publish</button> : null}{item.status === "published" ? <button className="secondary-button button-small" name="status" value="resolved">Resolve</button> : null}<button className="secondary-button button-small" name="status" value="archived">Archive</button></form></div> : null}
