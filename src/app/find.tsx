...
function GuideResult({ item, expanded, onPress }: { item: GuideItem; expanded: boolean; onPress: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.guideItem, expanded && styles.guideItemOpen, pressed && styles.pressed]}>
      ...
      <View style={styles.page}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.modePicker}>
            <Pressable onPress={() => setMode('guide')} style={[styles.mode, mode === 'guide' && styles.modeActive]}><Ionicons color={mode === 'guide' ? '#EFFFF8' : '#688184'} name="search-outline" size={17} /><Text style={[styles.modeText, mode === 'guide' && styles.modeTextActive]}>Bin guide</Text></Pressable>
            <Pressable onPress={() => setMode('services')} style={[styles.mode, mode === 'services' && styles.modeActive]}><Ionicons color={mode === 'services' ? '#EFFFF8' : '#688184'} name="map-outline" size={17} /><Text style={[styles.modeText, mode === 'services' && styles.modeTextActive]}>Local services</Text></Pressable>
          </View>
          ...
          {mode === 'guide' ? (
            ...
            <>
              ...
              <Pressable disabled={finding || !activeAddress} onPress={findServices} style={({ pressed }) => [styles.findButton, pressed && styles.pressed, (finding || !activeAddress) && styles.disabled]}>{finding ? <ActivityIndicator color="#FFFFFF" /> : <><Ionicons color="#FFFFFF" name="locate" size={18} /><Text style={styles.findButtonText}>{services ? 'Search again' : 'Find nearby services'}</Text></>}</Pressable>
...
const styles = StyleSheet.create({
  ...
  kicker: { color: '#B16E28', fontSize: 10, letterSpacing: 1.55, fontWeight: '900' },
  ...
  modePicker: { flexDirection: 'row', padding: 4, backgroundColor: '#DCE9E2', borderRadius: 15, gap: 3 },
  mode: { flex: 1, height: 42, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 7 },
  modeActive: { backgroundColor: '#0B766B' },
  modeText: { color: '#667F82', fontSize: 12, fontWeight: '800' },
  modeTextActive: { color: '#EFFFF8' },
  searchBox: { height: 51, borderRadius: 16, backgroundColor: '#FFFFFF', paddingHorizontal: 14, alignItems: 'center', flexDirection: 'row', gap: 9, shadowColor: '#16353A', shadowOpacity: 0.07, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 2 },
  ...
  chipsLabel: { color: '#74898B', fontSize: 9, letterSpacing: 1.1, fontWeight: '900', marginRight: 2 },
  ...
  sectionKicker: { color: '#6C8787', fontSize: 10, letterSpacing: 1.3, fontWeight: '900' },
  sectionTitle: { color: '#17383E', fontFamily: 'Georgia', fontSize: 20, letterSpacing: -0.35, marginTop: 4 },
  ...
  checkText: { color: '#956526', fontSize: 8, letterSpacing: 0.55, fontWeight: '900' },
  guideList: { borderRadius: 19, overflow: 'hidden', backgroundColor: '#FFFFFF', shadowColor: '#15343A', shadowOpacity: 0.07, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 2 },
  ...
  guideCopy: { flex: 1 },
  guideName: { color: '#1E4146', fontSize: 13.5, fontWeight: '900' },
  destination: { fontSize: 10.5, marginTop: 3, fontWeight: '800' },
  guideHeading: { color: '#274A4E', fontSize: 12, fontWeight: '900', marginTop: 10 },
  ...
  emptyTitle: { color: '#436267', fontSize: 13.5, fontWeight: '900', marginTop: 8 },
  ...
  servicesCopy: { flex: 1 },
  servicesTitle: { color: '#F3FFF8', fontSize: 15, fontWeight: '900' },
  ...
  locationCopy: { flex: 1 },
  locationLabel: { color: '#478078', fontSize: 9, letterSpacing: 1.1, fontWeight: '900' },
  ...
  findButtonText: { color: '#FFFFFF', fontSize: 13.5, fontWeight: '900' },
