export type CouncilCountry = 'England' | 'Northern Ireland' | 'Scotland' | 'Wales';
...
/**
 * UK local-authority district snapshot: ONS Local Authority Districts,
 * December 2024 boundaries. These are the collection-authority areas users
 * can resolve from a postcode; each needs a separately verified live adapter.
 */
...
const countryForCode: Record<string, CouncilCountry> = {
  E: 'England',
  N: 'Northern Ireland',
  S: 'Scotland',
  W: 'Wales',
...
export const councilDirectory: CouncilDirectoryEntry[] = rows.map(([code, name]) => ({
  code,
  name,
  country: countryForCode[code[0]],
  providerId: 'lad-' + code.toLowerCase(),
}));

export const councilDirectoryCounts = councilDirectory.reduce<Record<CouncilCountry, number>>(
  (counts, council) => ({ ...counts, [council.country]: counts[council.country] + 1 }),
...
function comparable(value: string) {
  return value
    .toLowerCase()
    .replace(/,\s*(city|district|county) of/g, '')
    .replace(/\b(the|council|borough|city|district|county)\b/g, '')
    .replace(/[^a-z0-9]/g, '');
}
...
export function searchCouncils(query: string) {
  const term = comparable(query);
  return term ? councilDirectory.filter((council) => comparable(council.name).includes(term)) : councilDirectory;
}
